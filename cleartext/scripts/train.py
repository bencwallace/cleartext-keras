...
def load_static(num_examples=1000, dataset='wikismall', vocab_size=10_000, embed_dim=50, train_frac=0.8):
    ...
    return train, dev, embed_matrix


def train(epochs=10, batch_size=32, num_examples=1000, dataset='wikismall', vocab_size=10000, embed_dim=50, units=32, train_frac=0.8):
    train, dev, embed_matrix = load_static(num_examples, dataset, vocab_size, embed_dim, train_frac)
    ...
    model.fit([train[0], train[1]], left_shift(train[1]), epochs=epochs, batch_size=batch_size, verbose=1)
